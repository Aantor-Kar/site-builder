import express from "express";
import "dotenv/config";
import cors from "cors";
import authRouter from "./routes/authRoutes.js";
import userRouter from "./routes/userRoutes.js";
import projectRouter from "./routes/projectRoutes.js";
import { stripeWebhooks } from "./controllers/stripeWebhooks.js";
const app = express();
function parseOrigins(raw) {
    return (raw ?? "")
        .split(",")
        .map((s) => s.trim().replace(/^['"]|['"]$/g, ""))
        .filter(Boolean);
}
function isDevLocalhostOrigin(origin) {
    return /^https?:\/\/(localhost|127\.0\.0\.1):\d+$/i.test(origin);
}
const corsOptions = {
    origin: (origin, callback) => {
        // Non-browser requests (curl, server-to-server) won't send Origin.
        if (!origin)
            return callback(null, true);
        const allowed = parseOrigins(process.env.TRUSTED_ORIGINS);
        const isExplicitAllow = allowed.includes(origin);
        const isWildcardLocalhostAllow = (allowed.includes("http://localhost:*") || allowed.includes("https://localhost:*") || allowed.includes("http://127.0.0.1:*") || allowed.includes("https://127.0.0.1:*")) &&
            isDevLocalhostOrigin(origin);
        return callback(null, isExplicitAllow || isWildcardLocalhostAllow);
    },
    credentials: true,
};
// Middleware
app.use(cors(corsOptions));
app.post('/api/stripe', express.raw({ type: 'application/json' }), stripeWebhooks);
app.use(express.json({ limit: '50mb' }));
const port = process.env.PORT || 3000;
app.use('/api/auth', authRouter);
app.use('/api/user', userRouter);
app.use('/api/project', projectRouter);
app.get('/', (req, res) => {
    res.send('Server is Live!');
});
app.listen(port, () => {
    console.log(`Server is running at http://localhost:${port}`);
});
